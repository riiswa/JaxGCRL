"""JAX Compatible version of MountainCarContinuous-v0 OpenAI gym environment with sparse reward.
...
class ContinuousMountainCar(environment.Environment[EnvState, EnvParams]):
    ...
    def step_env(
        ...
        # MODIFIED: Sparse reward - only provide reward of +1 when goal is reached
        # Removed the action penalty (-0.1 * action**2)
        goal_reached = (position >= params.goal_position) * (velocity >= params.goal_velocity)
        reward = 1.0 * goal_reached  # Convert to float, maintaining dtype consistency
    ...
    def discount(self, state: EnvState, params: EnvParams) -> jnp.ndarray:
        """Compute discount factor for current state."""
        return jnp.float32(~self.is_terminal(state, params))

    @property
    def name(self) -> str:
        """Environment name."""
        return "SparseMountainCarContinuous-v0"
